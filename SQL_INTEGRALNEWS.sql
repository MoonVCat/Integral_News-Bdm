...
BEGIN
	IF vOption = 'insertar' THEN
		INSERT INTO NEWS(`SIGN`, `TITLE`, `DESCRIPTION`, `TEXT_NEWS`, `CITY`, `SUBURB`, `COUNTRY`, `NEW_STATUS`, `DATE_OF_NEWS`, `HOUR_OF_NEWS`, `CREATION_DATE`, `CREATED_BY`, `LIKES`, `URGENTES` ) 
		VALUES(vSIGN, vTITLE, vDESCRIPTION, vTEXT_NEWS, vCITY, vSUBURB, vCOUNTRY, vNEW_STATUS, vDATE_OF_NEWS, vHOUR_OF_NEWS, sysdate(), vCREATED_BY, 0, vURGENTES);
	END IF;
    IF vOption = 'update' THEN
		UPDATE NEWS
			SET `SIGN` = vSIGN, `TITLE` = vTITLE, `DESCRIPTION` = vDESCRIPTION, `TEXT_NEWS` = vTEXT_NEWS, `CITY` = vCITY, `SUBURB` = vSUBURB, `COUNTRY` = vCOUNTRY, `NEW_STATUS` = vNEW_STATUS, `DATE_OF_NEWS` = vDATE_OF_NEWS, `HOUR_OF_NEWS` = vHOUR_OF_NEWS, `CREATION_DATE` = sysdate(), `URGENTES` = vURGENTES
...
CREATE TABLE `news_categories` (
  `N_CATE_ID` int(11) NOT NULL AUTO_INCREMENT COMMENT 'Llave primaria de la tabla NEWS_CATEGORIES',
  `NEWS_ID` int(11) NOT NULL COMMENT 'Id de la noticia a la que pertenece',
  `DESCRIPTION` varchar(100) CHARACTER SET utf8 COLLATE utf8_spanish_ci NOT NULL COMMENT 'Categorias de la noticia',
  `COLOR` varchar(100) CHARACTER SET utf8 COLLATE utf8_spanish_ci DEFAULT NULL,
...
BEGIN
	IF vOption = 'insertar' THEN
		set @news=(SELECT MAX(NEWS_ID) FROM NEWS);
		INSERT INTO NEWS_CATEGORIES(`NEWS_ID`, `DESCRIPTION`, `COLOR`, `CATE_ID`) 
		VALUES(@news, vDESCRIPTION, vCOLOR, vCATE_ID);
    END IF;
    IF vOption = 'editInsert' THEN
		INSERT INTO NEWS_CATEGORIES(`NEWS_ID`, `DESCRIPTION`, `COLOR`, `CATE_ID`) 
		VALUES(vNEWS_ID, vDESCRIPTION, vCOLOR, vCATE_ID);
    END IF;
    IF vOption = 'update' THEN
		UPDATE NEWS_CATEGORIES
			SET `NEWS_ID` = vNEWS_ID, `DESCRIPTION` = vDESCRIPTION, `COLOR` = vCOLOR , `CATE_ID` = vCATE_ID
		WHERE `NEWS_ID` = vNEWS_ID AND `N_CATE_ID` = vN_CATE_ID;
    END IF;
    IF vOption = 'updateCate' THEN
		UPDATE NEWS_CATEGORIES
			SET `DESCRIPTION` = vDESCRIPTION, `COLOR` = vCOLOR
		WHERE `CATE_ID` = vCATE_ID;
    END IF;
    IF vOption = 'delete' THEN
		DELETE FROM NEWS_CATEGORIES
		WHERE `NEWS_ID` = vNEWS_ID;
...
CREATE VIEW V_CATEGORIES_SELECT AS 
	SELECT C.CATEGORY_ID, C.`DESCRIPTION`, C.COLOR, C.`ORDER`, C.VIEWS, C.CREATION_DATE
...
CREATE VIEW V_NEWS_SELECT2 AS 
	SELECT N.NEWS_ID, N.SIGN, N.TITLE, N.`DESCRIPTION`, N.NEW_STATUS, N.CREATION_DATE, N.COMMENTS_EDITOR
	FROM NEWS AS N
	ORDER BY N.CREATION_DATE DESC;

CREATE VIEW V_NEWS_SELECT AS 
	SELECT N.NEWS_ID, N.SIGN, N.TITLE, N.`DESCRIPTION`, N.DATE_OF_NEWS, N.NEW_STATUS, N.CREATION_DATE, N.COMMENTS_EDITOR
	FROM NEWS AS N
	WHERE N.`NEW_STATUS` = 'Publicada'
	ORDER BY N.CREATION_DATE DESC;

CREATE VIEW V_NEWS_URGENTES AS 
	SELECT N.NEWS_ID, N.SIGN, N.TITLE, N.`DESCRIPTION`, N.DATE_OF_NEWS, N.NEW_STATUS, N.CREATION_DATE, N.COMMENTS_EDITOR, N.LIKES, N.URGENTES
	FROM NEWS AS N
	WHERE N.`NEW_STATUS` = 'Publicada' AND URGENTES = '1'
	ORDER BY N.CREATION_DATE DESC;

CREATE VIEW V_NEWS_POPULARES AS 
	SELECT N.NEWS_ID, N.SIGN, N.TITLE, N.`DESCRIPTION`, N.DATE_OF_NEWS, N.NEW_STATUS, N.CREATION_DATE, N.COMMENTS_EDITOR, N.LIKES
	FROM NEWS AS N
	WHERE N.`NEW_STATUS` = 'Publicada' 
	ORDER BY N.LIKES DESC;
    
CREATE VIEW V_CATEGORIES_SELECT2 AS 
	SELECT C.CATEGORY_ID, C.`DESCRIPTION`, C.COLOR
	FROM CATEGORIES AS C ;
    
    SELECT USER_ID, EMAIL, USER_TYPE_ID, USERNAME FROM USERS;
    
CREATE VIEW V_USERS_SELECT AS 
	SELECT U.USER_ID, U.`EMAIL`, U.USER_TYPE_ID, U.USERNAME
	FROM USERS AS U ;
    

SELECT AVG(`LIKES`) FROM NEWS;
...
FOR EACH ROW
BEGIN
set @newsId=(SELECT MAX(NEWS_ID) FROM NEWS);
set @repoId =(select CREATED_BY from NEWS where NEWS_ID=@newsId);
set @tituloNew =(select TITLE from NEWS where NEWS_ID=@newsId);

    INSERT INTO new_history(HISTORY_TEXT, CREATED_BY, NEWS_ID, `ACTION`)
    VALUES (@tituloNew, @repoId, @newsId, 'Insertar');
...
set @tituloNew =(select TITLE from NEWS where NEWS_ID=@newsId);
	...
	IF OLD.`NEW_STATUS` = 'Terminada' AND NEW.`NEW_STATUS` = 'Publicada' THEN
		INSERT INTO new_history(HISTORY_TEXT, CREATED_BY, NEWS_ID, `ACTION`)
		VALUES (@tituloNew, @repoId, @newsId, 'Aceptada');
    END IF;
...
BEGIN
	IF vOpcion = 'insertar' THEN
		...
		VALUES(vDescription, vColor, vORDER, CURDATE());
    END IF;
    IF vOpcion = 'update' THEN
		UPDATE CATEGORIES
